# SplineVST Expeirment on Compound Poisson-Gamma (Tweedie)
...
def tweedie_vst(y: np.ndarray, p: float, phi: float) -> np.ndarray:
    y_clipped = np.clip(y, a_min=0.0, a_max=None)
    scale = 1.0 / np.sqrt(max(phi, 1e-8))
    if abs(p - 2.0) < 1e-6:
        return scale * np.log(y_clipped + 1e-8)
    power = 1.0 - 0.5 * p
    return scale * (2.0 / (2.0 - p)) * np.power(y_clipped + 1e-8, power)


def simulate_tweedie_compound_poisson_gamma(
    num_samples: int,
    mu_range=(0.1, 100.0),
    p: float = 1.5,
    phi: float = 1.0,
    ...
    mu = np.exp(log_mu)
    p = float(np.clip(p, 1.01, 1.99))
    phi = float(max(phi, 1e-8))
    poisson_rate = mu ** (2.0 - p) / (phi * (2.0 - p))
    ...
    for i, n in enumerate(n_events):
        if n <= 0:
            continue
        y[i] = rng.gamma(shape=gamma_shape, scale=gamma_scale[i], size=n).sum()
...
def train_spline_vst(y: torch.Tensor):
    ...
    y = y.to(device)
    model = SplineVST(input_dim=1, num_knots=28, bandwidth_scale=1.0, grid_range=(-8.0, 8.0)).to(device)
    model.initialize(y)
...
def prepare_datasets(
    ...
    y_train, mu_train = simulate_tweedie_compound_poisson_gamma(
        train_size,
        mu_range=mu_range,
        p=p,
        phi=phi,
    ...
    y_test, mu_test = simulate_tweedie_compound_poisson_gamma(
        test_size,
        mu_range=mu_range,
        p=p,
        phi=phi,
...
def plot_transforms_matched_scale(
    ...
    """Plot transforms with all methods rescaled to match Tweedie VST's output range."""
    tweedie_curve = tweedie_vst(x_plot, p=p, phi=phi)
    tweedie_min, tweedie_max = tweedie_curve.min(), tweedie_curve.max()

    def rescale_to_tweedie(curve):
        """Rescale any curve to match Tweedie VST's output range."""
        c_min, c_max = curve.min(), curve.max()
        normalized = (curve - c_min) / (c_max - c_min + 1e-8)
        return normalized * (tweedie_max - tweedie_min) + tweedie_min
    ...
    spline_rescaled = rescale_to_tweedie(spline_curve)
    spline_self_consistent_rescaled = rescale_to_tweedie(spline_self_consistent_curve)
    spline_oracle_rescaled = rescale_to_tweedie(spline_oracle_curve)
    ...
    if convex_fn_raw is not None and bin_centers_raw is not None:
        ...
        convex_raw_rescaled = rescale_to_tweedie(convex_raw_curve)

    # Plot
    plt.figure(figsize=(8, 5))
    plt.plot(x_plot, rescale_to_tweedie(x_plot), label='Identity', linestyle=':', alpha=0.5, color='gray')
    plt.plot(x_plot, tweedie_curve, label='Tweedie VST', linewidth=2)
...
def plot_derivatives_matched_scale(
    ...
    """Plot transform derivatives with all methods rescaled to match Tweedie VST's output range."""
    ...
    # Compute Tweedie VST curve and its derivative
    tweedie_curve = tweedie_vst(x_plot, p=p, phi=phi)
    tweedie_deriv = numeric_derivative(tweedie_curve, x_plot)

    # Classical VST derivative: 1/sqrt(phi * y^p)
    eps = 1e-6
    y_safe = np.clip(x_plot, a_min=eps, a_max=None)
    classical_deriv = 1.0 / np.sqrt(phi * (y_safe**p))
    ...
    spline_rescaled = rescale_to_match(spline_curve, tweedie_curve)
    spline_deriv = numeric_derivative(spline_rescaled, x_plot)
    spline_self_consistent_rescaled = rescale_to_match(spline_self_consistent_curve, tweedie_curve)
    spline_self_consistent_deriv = numeric_derivative(spline_self_consistent_rescaled, x_plot)
    spline_oracle_rescaled = rescale_to_match(spline_oracle_curve, tweedie_curve)
    ...
    if convex_fn_raw is not None and bin_centers_raw is not None:
        convex_raw_curve = apply_convexvst(x_plot, convex_fn_raw, bin_centers_raw)
        convex_raw_rescaled = rescale_to_match(convex_raw_curve, tweedie_curve)
        convex_raw_deriv = numeric_derivative(convex_raw_rescaled, x_plot)

    # Plot
    plt.figure(figsize=(8, 5))
    plt.plot(x_plot, classical_deriv, label='Classical 1/√(φ y^p)', linestyle='--', color='black', linewidth=2)
    plt.plot(x_plot, tweedie_deriv, label="Tweedie VST T'(y)", linewidth=2)
...
def plot_residuals_diagnostic(
    ...
    transforms = {
        ...
        'Tweedie VST': tweedie_test[sort_idx],
...
def main():
    p = 1.5
    phi = 1.0
    (y_train, mu_train), (y_test, mu_test) = prepare_datasets(
        train_size=20000,
        test_size=10000,
        p=p,
        phi=phi,
    ...
    tweedie_test = tweedie_vst(y_test, p=p, phi=phi)
    ...
    tweedie_metrics = evaluate_with_bootstrap(tweedie_test, mu_test, num_bins=20, num_resamples=200, rng=rng)
    ...
    tweedie_strata = stratify_metrics(tweedie_test, mu_test)
    ...
    print_metrics('Tweedie VST', tweedie_metrics)
    ...
    print_strata('Tweedie VST', tweedie_strata)
    ...
    plt.plot(tweedie_metrics['bin_centers'], tweedie_metrics['bin_vars'], label='Tweedie VST variance', marker='o')
    ...
    plt.xscale('log')
    plt.yscale('log')
    plt.xlabel('Tweedie mean')
    plt.ylabel('Transformed variance per μ bin (test)')
    plt.title('Variance stabilization on Tweedie data (test set)')
    plt.legend()
    plt.tight_layout()
    ...
    plt.plot(x_plot, normalize(tweedie_curve), label='Tweedie VST')
    ...
    plt.subplot(1, 2, 1)
    plt.plot(x_plot, tweedie_curve, label='Tweedie VST')
    ...
    plt.plot(x_plot[mask], tweedie_curve[mask], label='Tweedie VST')
