# SplineVST Expeirment on Negative Binomial
...
class SplineVST(nn.Module):
    ...
    def __init__(
        ...
        proxy_clip: float = 100.0,
        enable_superlinear_proxy: bool = False,
        ...
        self.proxy_clip = proxy_clip
        self.enable_superlinear_proxy = enable_superlinear_proxy
        ...
        self.register_buffer('proxy_mode', torch.zeros(input_dim, dtype=torch.long))
    ...
    def _proxy_mode_code(self, mode: str) -> int:
        if mode == 'log1p':
            return self._PROXY_LOG1P
        if mode == 'log1p_clip':
            return self._PROXY_LOG1P_CLIP
        if mode == 'asinh':
            return self._PROXY_ASINH
        if mode == 'linear':
            return self._PROXY_LINEAR
        raise ValueError(f'Unknown proxy mode: {mode}')

    def _decide_proxy_mode(self, x):
        if self.requested_bin_mode in {'log1p', 'linear', 'log1p_clip', 'asinh'}:
            mode = self._proxy_mode_code(self.requested_bin_mode)
            return torch.full((self.input_dim,), mode, dtype=torch.long, device=x.device)
        ...
        ratio = ranges / (means.abs() + 1e-6)
        superlinear = self.enable_superlinear_proxy & (ratio > self.superlinear_ratio_threshold)

        modes = torch.full((self.input_dim,), self._PROXY_LINEAR, dtype=torch.long, device=x.device)
        modes = torch.where(nonnegative & high_range, self._PROXY_LOG1P, modes)
        if self.enable_superlinear_proxy:
            modes = torch.where(superlinear, self._proxy_mode_code(self.superlinear_proxy_mode), modes)
        return modes
    ...
    def _apply_proxy(self, x):
        ...
        mode = self.proxy_mode.view(1, -1)
        x_clamped = x.clamp_min(0.0)
        log1p = torch.log1p(x_clamped)
        log1p_clip = torch.log1p(x_clamped.clamp_max(self.proxy_clip))
        scale = max(self.proxy_clip, 1e-6)
        asinh = torch.asinh(x / scale)

        out = torch.where(mode == self._PROXY_LOG1P, log1p, x)
        out = torch.where(mode == self._PROXY_LOG1P_CLIP, log1p_clip, out)
    ...
    def init_bins_from_data(self, x_input):
        with torch.no_grad():
            ...
            proxy_modes = self._decide_proxy_mode(x_input)
            self.proxy_mode.copy_(proxy_modes)
            device = self.grid.device
            q_steps = torch.linspace(0.01, 0.99, self.num_bins, device=device)

            if self.bin_centers_from_proxy:
                x_proxy = self._apply_proxy(x_input)
                z_proxy = self._rank_gauss(x_proxy)
                centers = torch.quantile(z_proxy, q_steps, dim=0).T
                self.bin_centers.copy_(centers)
...
def negative_binomial_vst(y: np.ndarray, dispersion: float) -> np.ndarray:
    ...
    return 2.0 * np.sqrt(r) * np.arcsinh(np.sqrt(y_clipped / r))
...
def prepare_datasets(train_size=20000, test_size=10000, dispersion=10.0, mu_range=(0.1, 100.0), seed=0):
    rng = np.random.default_rng(seed)
    y_train, mu_train = simulate_negative_binomial(
        train_size, mu_range=mu_range, dispersion=dispersion, rng=rng
    )
    y_test, mu_test = simulate_negative_binomial(
        test_size, mu_range=mu_range, dispersion=dispersion, rng=rng
    )
...
def plot_transforms_matched_scale(
    ...
    """Plot transforms with all methods rescaled to match the negative binomial VST."""
    nb_curve = negative_binomial_vst(x_plot, dispersion)
    nb_min, nb_max = nb_curve.min(), nb_curve.max()

    def rescale_to_nb(curve):
        """Rescale any curve to match the negative binomial VST range."""
        c_min, c_max = curve.min(), curve.max()
        normalized = (curve - c_min) / (c_max - c_min + 1e-8)
        return normalized * (nb_max - nb_min) + nb_min
...
def plot_derivatives_matched_scale(
    ...
    if convex_fn_raw is not None and bin_centers_raw is not None:
        convex_raw_curve = apply_convexvst(x_plot, convex_fn_raw, bin_centers_raw)
        convex_raw_rescaled = rescale_to_match(convex_raw_curve, nb_curve)
...
def main():
    ...
    (y_train, mu_train), (y_test, mu_test) = prepare_datasets(
        train_size=20000, test_size=10000, dispersion=dispersion, seed=0
    )
    ...
    plt.xscale('log')
    plt.yscale('log')
    plt.xlabel('Negative binomial mean')
